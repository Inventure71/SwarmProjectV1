...
class BackendServer:
    ...
    def __init__(self) -> None:
        ...
        self.path_followers: Dict[str, PathFollower] = {}
        self.paths: Dict[str, PathData] = {}
        self.racing_configs: Dict[str, RacingConfig] = {}
        self.last_throttle: Dict[str, float] = {}
    ...
    def _remove_robot_internal(self, name: str, persist: bool = True) -> None:
        with self._state_lock:
            ...
            self.racing_configs.pop(name, None)
            self.paths.pop(name, None)
            follower = self.path_followers.pop(name, None)
            self.last_throttle.pop(name, None)
    ...
    def _control_loop(self) -> None:
        ...
        while not self._stop_event.is_set() and not rospy.is_shutdown():
            ...
            with self._state_lock:
                active_followers = dict(self.path_followers)

            for robot_name, follower in active_followers.items():
                robot = self.robots.get(robot_name)
                if not robot:
                    self._stop_following(robot_name, reason="missing robot")
                    continue

                pose = robot.get_position()
                follower.update_position(*pose)

                if follower.is_complete():
                    self._stop_following(robot_name, reason="path complete")
                    continue

                throttle, turn_rate = follower.compute_command()
                throttle, turn_rate = self._apply_speed_controls(robot_name, follower, throttle, turn_rate)
                self.controller.send_command(robot_name, throttle, -turn_rate)
                self.last_throttle[robot_name] = throttle
    ...
    def _apply_speed_controls(
        ...
        distance_to_waypoint = state.get("distance_to_target") or 999.0

        if throttle > 0.01 and distance_to_waypoint < params["look_ahead_distance"]:
            idx = state.get("waypoint_index", 0)
            waypoints = follower.waypoints
            if idx + 1 < len(waypoints):
                x, y, yaw = follower.current_position or (0.0, 0.0, 0.0)
        ...
        if ramp_rate > 1e-6:
            last_throttle = self.last_throttle.get(robot_name, 0.0)
    ...
    def _stop_following(self, robot_name: str, reason: str = "stopped") -> None:
        with self._state_lock:
            follower = self.path_followers.pop(robot_name, None)
        if follower:
            follower.reset()
        self.controller.send_command(robot_name, 0.0, 0.0)
    ...
    def _broadcast_loop(self) -> None:
        ...
        while not self._stop_event.is_set() and not rospy.is_shutdown():
            ...
            with self._state_lock:
                ...
                followers_snapshot = dict(self.path_followers)
            ...
            for name, robot in robots_snapshot.items():
                ...
                robot_states[name] = {
                    ...
                    "is_following": name in followers_snapshot,
                }

            self.udp_server.broadcast({"type": "robot_states", "data": robot_states})

            follower_states = {}
            for name, follower in followers_snapshot.items():
                state = follower.get_state()
                follower_states[name] = {
                    ...
                    "throttle": self.last_throttle.get(name, 0.0),
                    "is_complete": follower.is_complete(),
                }
            if follower_states:
    ...
    def _cmd_set_path(self, data: Dict, addr: Tuple[str, int]) -> None:
        ...
        with self._state_lock:
            self.paths[name] = PathData(waypoints=processed, updated_at=time.time())
        self._send_ack(addr, "set_path", {"num_waypoints": len(processed)})
    ...
    def _cmd_clear_path(self, data: Dict, addr: Tuple[str, int]) -> None:
        ...
        with self._state_lock:
            self.paths.pop(name, None)
        self._stop_following(name, reason="path cleared")
    ...
    def _cmd_start_all_paths(self, data: Dict, addr: Tuple[str, int]) -> None:
        started = 0
        for name in list(self.paths.keys()):
    ...
    def _cmd_set_racing_config(self, data: Dict, addr: Tuple[str, int]) -> None:
        ...
        if not name:
            raise ValueError("'robot' is required")
        cfg = self.racing_configs.get(name)
        if not cfg:
            raise ValueError(f"Robot '{name}' not registered")

        if "offset" in data:
            cfg.set_offset(float(data["offset"]))
        if "speed" in data:
            cfg.set_speed_multiplier(float(data["speed"]))
        if "loop" in data:
            cfg.set_loop(bool(data["loop"]))

        follower = self.path_followers.get(name)
        if follower:
            follower.set_lateral_offset(cfg.lateral_offset)
            follower.speed_multiplier = cfg.speed_multiplier
    ...
    def _cmd_set_parameters(self, data: Dict, addr: Tuple[str, int]) -> None:
        ...
        if updated:
            with self._state_lock:
                for follower in self.path_followers.values():
                    follower.use_prediction = bool(self.parameters["use_prediction"])
                    follower.estimated_delay_ms = float(self.parameters["estimated_delay_ms"])
    ...
    def _cmd_manual_control(self, data: Dict, addr: Tuple[str, int]) -> None:
        ...
        self._stop_following(name, reason="manual override")
        ...
        if robot.robot_type == "dummy":
            robot.set_command(float(throttle), float(turn_rate))
        else:
    ...
    def _start_path_for_robot(self, name: str) -> bool:
        with self._state_lock:
            if name in self.path_followers:
                return False
            path_data = self.paths.get(name)
        ...
        params = self.parameters
        follower = PathFollower(
            waypoints=list(path_data.waypoints),
        ...
        with self._state_lock:
            self.path_followers[name] = follower
        rospy.loginfo("[Backend] Started path follower for %s (%d waypoints)", name, len(path_data.waypoints))
        return True
