...
class RobotConfig:
    ...
    def should_flip_pose_x(self) -> bool:
        """
        Return whether pose X should be mirrored into Hydra's world frame.
...
def load_fleet_config(config_path: str | Path | None = None) -> FleetConfig:
    ...
    for name, cfg in raw.get("ROBOT_CONFIG", {}).items():
        ...
        robots[name] = RobotConfig(
            ...
            max_linear=float(cfg.get("max_linear", raw.get("HYDRA_CONFIG", {}).get("default_max_linear", 0.5))),
            max_angular=float(cfg.get("max_angular", raw.get("HYDRA_CONFIG", {}).get("default_max_angular", 1.5))),
    ...
    return FleetConfig(
        robots=robots,
        hydra_config=dict(raw.get("HYDRA_CONFIG", {})),
