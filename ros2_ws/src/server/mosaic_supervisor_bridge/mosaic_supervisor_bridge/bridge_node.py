...
from mosaic_common import RacingConfig, load_fleet_config
from mosaic_interfaces.action import FollowPath
from mosaic_interfaces.msg import PathProgress, RobotStatus
...
class MosaicSupervisorBridge(Node):
    def __init__(self) -> None:
        super().__init__("mosaic_supervisor_bridge")
        ...
        mosaic_cfg = self.fleet.mosaic_config
        self.supervisor_host = str(mosaic_cfg.get("supervisor_host", "127.0.0.1"))
        self.supervisor_port = int(mosaic_cfg.get("supervisor_port", 9998))
        self.broadcast_hz = float(mosaic_cfg.get("state_broadcast_hz", 20))
        self.heartbeat_hz = float(mosaic_cfg.get("supervisor_heartbeat_hz", 2.0))
        ...
        self.heartbeat_pub = self.create_publisher(Header, "/mosaic/supervisor_heartbeat", 10)
        ...
        for name, robot in self.fleet.robots.items():
            ...
            self.create_subscription(RobotStatus, f"{robot.namespace}/mosaic/status", lambda msg, robot_name=name: self._on_status(robot_name, msg), 10)
            self.create_subscription(PathProgress, f"{robot.namespace}/mosaic/path_progress", lambda msg, robot_name=name: self._on_progress(robot_name, msg), 10)
            self.follow_actions[name] = ActionClient(self, FollowPath, f"{robot.namespace}/mosaic/follow_path")
            self.mode_clients[name] = self.create_client(SetMode, f"{robot.namespace}/mosaic/set_mode")
            self.racing_clients[name] = self.create_client(SetRacingConfig, f"{robot.namespace}/mosaic/set_racing_config")
            self.estop_clients[name] = self.create_client(Trigger, f"{robot.namespace}/mosaic/emergency_stop")
            self.cancel_clients[name] = self.create_client(Trigger, f"{robot.namespace}/mosaic/cancel_motion")
            self.clear_clients[name] = self.create_client(Trigger, f"{robot.namespace}/mosaic/clear_path")
