...
from mosaic_common import PathFollower, RacingConfig, load_fleet_config
from mosaic_interfaces.action import FollowPath
from mosaic_interfaces.msg import PathProgress, RobotDiagnostics, RobotStatus
from mosaic_interfaces.srv import SetMode, SetRacingConfig
...
class MosaicRobotAgent(Node):
    ...
    def __init__(self) -> None:
        super().__init__("mosaic_robot_agent")
        ...
        self.robot = robot
        self.mosaic_cfg = fleet.mosaic_config
        ...
        self.pose_timeout = float(self.mosaic_cfg.get("pose_stale_timeout_sec", 0.5))
        self.heartbeat_timeout = float(self.mosaic_cfg.get("supervisor_heartbeat_timeout_sec", 1.0))
        self.control_hz = float(self.mosaic_cfg.get("path_control_hz", 20.0))
        self.status_hz = float(self.mosaic_cfg.get("status_publish_hz", 10.0))
        ...
        self.status_pub = self.create_publisher(RobotStatus, f"{self.robot.namespace}/mosaic/status", 10)
        self.progress_pub = self.create_publisher(PathProgress, f"{self.robot.namespace}/mosaic/path_progress", 10)
        self.pose_pub = self.create_publisher(PoseStamped, f"{self.robot.namespace}/mosaic/pose", 10)
        self.diag_pub = self.create_publisher(RobotDiagnostics, f"{self.robot.namespace}/mosaic/diagnostics", 10)

        self.create_subscription(Header, "/mosaic/supervisor_heartbeat", self._on_heartbeat, 10)
        ...
        self.mode_srv = self.create_service(SetMode, f"{self.robot.namespace}/mosaic/set_mode", self._handle_set_mode, callback_group=self.cb_group)
        self.racing_srv = self.create_service(SetRacingConfig, f"{self.robot.namespace}/mosaic/set_racing_config", self._handle_set_racing, callback_group=self.cb_group)
        self.estop_srv = self.create_service(Trigger, f"{self.robot.namespace}/mosaic/emergency_stop", self._handle_estop, callback_group=self.cb_group)
        self.cancel_srv = self.create_service(Trigger, f"{self.robot.namespace}/mosaic/cancel_motion", self._handle_cancel, callback_group=self.cb_group)
        self.clear_srv = self.create_service(Trigger, f"{self.robot.namespace}/mosaic/clear_path", self._handle_clear, callback_group=self.cb_group)
        self.action_server = ActionServer(
            self,
            FollowPath,
            f"{self.robot.namespace}/mosaic/follow_path",
        ...
        self.get_logger().info(f"Mosaic robot agent ready for {self.robot.name}")
        if self.pose_flip_x:
