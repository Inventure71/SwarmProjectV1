...
from src.services.racing_config import RacingConfig
...
class RobotControllerApp:
    ...
    def _load_config(self):
        ...
        print(f"[Controller] Initializing tracker for {len(robot_config)} robots...")
        self.tracker = RobotTracker(robot_config)
        self.robots = self.tracker.robots
        
        if not self.robots:
            print(f"[Controller] ✗ No robots found in config!")
            sys.exit(1)
        
        self.active_robot = list(self.robots.keys())[0]
        
        for robot_name in self.robots.keys():
            self.racing_configs[robot_name] = RacingConfig(robot_name)
        
        self.controller = RobotController(host='0.0.0.0', port=6969, robots=self.robots)
        
        self.use_prediction = True
        self.estimated_delay_ms = 100
        self.waypoint_tolerance = 0.20
        self.turn_in_place_threshold = 65.0
        self.proportional_gain = 2.8
        self.max_turn_rate = 85.0
        self.slow_down_distance = 0.5
        self.min_speed_ratio = 0.20
        self.curvature_speed_gain = 0.65
        ...
        self.waypoint_approach_slowdown = 0.30
        self.corner_keep_angle_deg = 22.0
        self.intermediate_corner_slowdown_deg = 90.0
        self.throttle_ramp_rate = 0.9
        
    def _setup_ui(self):
        """Setup the user interface."""
        self.root = tk.Tk()
        self.root.title(f"🤖 Robot Swarm Controller")
    ...
    def _setup_robot_selector(self, parent):
        ...
        robot_dropdown = tk.OptionMenu(robot_frame, self.robot_var, *self.robots.keys(), 
                                       command=self._on_robot_change)
        robot_dropdown.config(bg='#2a2a2a', fg='#fff', font=('Arial', 10), 
                             highlightthickness=0, activebackground='#00ff88')
        robot_dropdown.pack(fill=tk.X, padx=8, pady=(8,5))
        
        race_cfg_frame = tk.Frame(robot_frame, bg='#1a1a1a')
        race_cfg_frame.pack(fill=tk.X, padx=8, pady=5)
        
        tk.Label(race_cfg_frame, text="Offset:", bg='#1a1a1a', fg='#ccc', 
                font=('Arial', 8)).grid(row=0, column=0, sticky=tk.W)
        self.offset_var = tk.DoubleVar(value=0.0)
        offset_scale = ModernScale(race_cfg_frame, from_=-0.5, to=0.5, orient=tk.HORIZONTAL,
                                   variable=self.offset_var, command=self._on_offset_change,
                                   resolution=0.05)
        offset_scale.grid(row=0, column=1, sticky=tk.EW, padx=5)
        self.offset_label = tk.Label(race_cfg_frame, text="0.0m", bg='#1a1a1a', fg='#00ff88',
                                     font=('Arial', 8), width=6)
        self.offset_label.grid(row=0, column=2)
        
        tk.Label(race_cfg_frame, text="Speed:", bg='#1a1a1a', fg='#ccc',
                font=('Arial', 8)).grid(row=1, column=0, sticky=tk.W, pady=(3,0))
        self.speed_var = tk.DoubleVar(value=1.0)
        speed_scale = ModernScale(race_cfg_frame, from_=0.3, to=1.5, orient=tk.HORIZONTAL,
                                 variable=self.speed_var, command=self._on_speed_change,
                                 resolution=0.1)
        speed_scale.grid(row=1, column=1, sticky=tk.EW, padx=5, pady=(3,0))
        self.speed_label = tk.Label(race_cfg_frame, text="1.0x", bg='#1a1a1a', fg='#00ff88',
                                    font=('Arial', 8), width=6)
        ...
        loop_check = ModernCheckbutton(robot_frame, text="Loop Circuit",
                                       variable=self.loop_var, command=self._on_loop_change)
    ...
    def _setup_services(self):
        ...
        self.recording_service = RecordingService(
            self.controller, self.tracker, self.robots[self.active_robot], self.canvas, self._world_to_canvas
        )
    ...
    def _update_path_status(self):
        ...
        total_robots_with_paths = sum(1 for r in self.robots.keys() if self.path_service.has_path(r))
        status_text = f"{mode_name} | {self.active_robot}: {info['num_waypoints']} pts"
        if total_robots_with_paths > 1:
            status_text += f" | {total_robots_with_paths} robots have paths"
    ...
    def _start_recording(self):
        ...
        if self.recording_service.start_recording():
            ...
            print(f"[Recording] Started motion recording for {self.active_robot}")
    ...
    def _stop_recording(self):
        ...
        if len(recorded_positions) > 0:
            self.path_service.set_recorded_path(self.active_robot, recorded_positions)
            self.cmd_label.config(
                text=f"🎮 {self.active_robot}: {len(recorded_positions)} points recorded",
                fg='#00ff88'
            )
            self._update_button_states()
            print(f"[Recording] Captured {len(recorded_positions)} waypoints for {self.active_robot}")
        else:
    ...
    def _load_path(self):
        ...
        if self.path_service.load_path(self.active_robot):
            info = self.path_service.get_path_info(self.active_robot)
            self.cmd_label.config(text=f"✓ {self.active_robot}: {info['num_waypoints']} waypoints loaded", fg='#00ff88')
            self._update_button_states()
    
    def _clear(self):
        """Clear path for active robot."""
        if self.active_robot in self.robot_followers:
            self.controller.send_command(0.0, 0.0, self.active_robot)
            self.robot_followers[self.active_robot].reset()
            del self.robot_followers[self.active_robot]
            if not self.robot_followers:
                self.is_animating = False
    ...
    def _connect(self):
        ...
        self.status_label.config(text=f"🟡 Waiting for robots...", fg='#ffaa00')
    ...
    def _check_connection_status(self):
        ...
        with self.controller.clients_lock:
            num_connected = len(self.controller.client_sockets)
            connected_robots = list(self.controller.client_sockets.keys())
        
        num_dummy = sum(1 for r in self.robots.values() if r.robot_type == 'dummy')
        num_real = len([r for r in self.robots.values() if r.robot_type == 'real'])
        
        current_status = (num_connected, tuple(sorted(connected_robots)))
        if current_status != self._last_connection_status:
            print(f"[Controller] 🔄 Connection status changed:")
            print(f"[Controller]    Connected robots: {connected_robots}")
            print(f"[Controller]    {num_connected}/{num_real} real, {num_dummy} dummy")
            self._last_connection_status = current_status
        
        if num_connected > 0 or num_dummy > 0:
            if num_connected == num_real:
                status_text = f"🟢 Connected: {num_connected} real, {num_dummy} dummy"
                self.status_label.config(text=status_text, fg='#00ff88')
            else:
                status_text = f"🟡 Partial: {num_connected}/{num_real} real, {num_dummy} dummy"
                self.status_label.config(text=status_text, fg='#ffaa00')
        else:
            self.status_label.config(text=f"🟡 Waiting for {num_real} real robots...", fg='#ffaa00')
    ...
    def _update_position(self):
        ...
        robot = self.robots[self.active_robot]
        x, y, yaw = robot.get_position()
        self._draw_robots()
        
        yaw_deg = math.degrees(yaw)
        robot_type = "🤖" if robot.robot_type == 'real' else "🎮"
        self.info_label.config(text=f"{robot_type} {self.active_robot}: ({x:6.3f}, {y:6.3f})m | {yaw_deg:6.1f}°")
    ...
    def _draw_robots(self):
        """Draw all robots."""
        for tag in list(self.robot_visuals.values()):
        ...
        for name, robot in self.robots.items():
            ...
            self.canvas.create_line(x, y, arrow_x, arrow_y,
                                   fill='#fff', width=3 if is_active else 2, 
                                   arrow=tk.LAST, tags='robot_arrow')
            
            label_y = y - size - 10
            label_text = name if is_active else name[:6]
            self.canvas.create_text(x, label_y, text=label_text,
                                   fill='#fff' if is_active else '#888',
                                   font=('Arial', 9 if is_active else 7, 'bold'),
                                   tags='robot_label')
            
            if is_active:
                follower = self.robot_followers.get(name)
                if follower and hasattr(follower, 'predictor') and self.use_prediction:
                    pred_x_m, pred_y_m, pred_yaw = follower.predictor.predict(
    ...
    def _start(self):
        ...
        follower.loop_enabled = cfg.loop_path
        
        self.robot_followers[self.active_robot] = follower
        
        if not self.is_animating:
            self.is_animating = True
            self.last_control_time = time.time()
            if not hasattr(self, 'last_throttle_commands'):
                self.last_throttle_commands = {}
            self._control_loop()
        
        self.last_throttle_commands[self.active_robot] = 0.0
    ...
    def _control_loop(self):
        ...
        for robot_name, follower in list(self.robot_followers.items()):
            if follower.is_complete():
                self.controller.send_command(0.0, 0.0, robot_name)
                del self.robot_followers[robot_name]
                continue
            ...
            if throttle > 0.01 and distance_to_waypoint < self.look_ahead_distance:
                ...
                if current_idx + 1 < state['total_waypoints']:
                    next_target = follower.waypoints[current_idx + 1]
            ...
            if self.throttle_ramp_rate > 1e-6:
                last_throttle = self.last_throttle_commands.get(robot_name, 0.0)
                ...
                if delta > max_delta:
                    ...
                    throttle = last_throttle - max_delta
                self.last_throttle_commands[robot_name] = throttle

            self.controller.send_command(throttle, -turn_rate, robot_name)
        ...
        if not self.robot_followers:
            self._stop()
            self.cmd_label.config(text="✓ All paths complete!", fg='#00ff88')
            return
    ...
    def _emergency(self):
        ...
        for follower in self.robot_followers.values():
            follower.reset()
        self.robot_followers.clear()
        
        self.start_btn.config(state=tk.NORMAL)
        self.stop_btn.config(state=tk.DISABLED)
        self.cmd_label.config(text="🛑 EMERGENCY", fg='#f44336')
    
    def _on_robot_change(self, robot_name):
        """Handle active robot change."""
        ...
        self._update_button_states()
        self._update_path_status()
        print(f"[Controller] Active robot changed to: {robot_name}")
    
    def _on_offset_change(self, value):
        """Handle offset change."""
        offset = float(value)
        self.racing_configs[self.active_robot].set_offset(offset)
        self.offset_label.config(text=f"{offset:.2f}m")
        
        if self.active_robot in self.robot_followers:
            follower = self.robot_followers[self.active_robot]
            follower.set_lateral_offset(offset)
    
    def _on_speed_change(self, value):
        """Handle speed multiplier change."""
        speed = float(value)
        self.racing_configs[self.active_robot].set_speed_multiplier(speed)
        self.speed_label.config(text=f"{speed:.1f}x")
        
        if self.active_robot in self.robot_followers:
            self.robot_followers[self.active_robot].speed_multiplier = speed
    
    def _on_loop_change(self):
        """Handle loop toggle."""
        loop = self.loop_var.get()
        self.racing_configs[self.active_robot].set_loop(loop)
        
        if self.active_robot in self.robot_followers:
            self.robot_followers[self.active_robot].loop_enabled = loop
    
    def _update_button_states(self):
        """Update button states based on active robot status."""
        has_path = self.path_service.has_path(self.active_robot)
        is_following = self.active_robot in self.robot_followers
        ...
        any_following = len(self.robot_followers) > 0
    ...
    def _start_all(self):
        ...
        for robot_name in self.robots.keys():
            ...
            follower.loop_enabled = cfg.loop_path
            self.robot_followers[robot_name] = follower
            robots_started.append(robot_name)
        ...
        self.cmd_label.config(text=f"▶ {len(robots_started)} robots following paths...", fg='#00ff88')
    ...
    def _on_delay_change(self, value):
        ...
        for follower in self.robot_followers.values():
            follower.estimated_delay_ms = self.estimated_delay_ms
    
    def _on_setting_change(self):
        """Handle setting change."""
        self.use_prediction = self.prediction_var.get()
        for follower in self.robot_followers.values():
            follower.use_prediction = self.use_prediction
    ...
    def _get_debug_info(self):
        ...
        follower = self.robot_followers.get(self.active_robot)
        if follower and hasattr(follower, 'predictor'):
            pred_x, pred_y, pred_yaw = follower.predictor.predict(
